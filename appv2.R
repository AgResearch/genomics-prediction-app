...
server <- function(input, output, session) {
  ...
  output$tableS1 <- renderDT({
    ...
    cat("Actual Table S1 columns:", names(supp_data$table_s1), "\n")
    
    # Process the table to create ± format - handle both possible column structures
    if ("Accuracy" %in% names(supp_data$table_s1)) {
      # Your actual data structure
      processed_table <- supp_data$table_s1 %>%
        mutate(
          `Genomic Prediction Accuracy` = paste0(
            sprintf("%.3f", as.numeric(Accuracy)), 
            " ± ", 
            sprintf("%.3f", as.numeric(Accuracy_SE))
          ),
          `Dispersion Bias` = paste0(
            sprintf("%.3f", as.numeric(Bias)), 
            " ± ", 
            sprintf("%.3f", as.numeric(Bias_SE))
          )
        ) %>%
        select(
      ...
      # Fallback: just return the raw data if columns are different
      processed_table <- supp_data$table_s1
    }
  ...
  output$tableS2 <- renderDT({
    ...
    # Debug: Check what columns we actually have
    cat("Actual Table S2 columns:", names(supp_data$table_s2), "\n")
    
    # Process the table to create ± format - handle both possible column structures
    if ("Accuracy" %in% names(supp_data$table_s2)) {
      # Your actual data structure
